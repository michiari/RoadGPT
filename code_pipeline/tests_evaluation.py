...
def _interpolate_and_resample_splines(sample_nodes, height, nodes_per_meter = 1, smoothness=0, k=3, rounding_precision=4):
...
class RoadTestEvaluator:
    ...
    def identify_interesting_road_segments(self, road_nodes, execution_data, height):
        # Interpolate and resample
        # TODO we also have already interpolated points
        road_points = _interpolate_and_resample_splines(road_nodes, height)
...
class OOBAnalyzer:
    ...
    """
    def __init__(self, result_folder, road_length_before_oob=30, road_length_after_oob=30, height=-28.0):
        self.logger = logging.getLogger('OOBAnalyzer')
        self.oobs = self._load_oobs_from(result_folder, road_length_before_oob, road_length_after_oob, height)

    def _load_oobs_from(self, result_folder, road_length_before_oob, road_length_after_oob, height):
            ...
            for sample_file in sorted(
                ...
                try:
                    ...
                    oob_pos, segment_before, segment_after, oob_side, oob_idx, execution_before = \
                        road_test_evaluation.identify_interesting_road_segments(road_data, execution_data, height)
